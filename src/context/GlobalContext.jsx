...
export const MoviesProvider = ({ children }) => {
    ...
    useEffect(() => {
        fetch(`https://api.themoviedb.org/3/trending/movie/day?api_key=${api_key}`)
            .then((res) => res.json())
            .then(({ results }) => {
                const normalizedMovies = results.map(movie => normalizeData(movie, 'movie'));
    ...
    const searchContents = async () => {
        ...
        try {
            // Fetch parallelo per film e serie TV
            ...
            // Normalizza e combina i risultati
            const normalizedMovies = movieData.results.map(movie => normalizeData(movie, 'movie'));
